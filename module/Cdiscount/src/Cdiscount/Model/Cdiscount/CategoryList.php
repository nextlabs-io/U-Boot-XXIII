...
class CategoryList extends HtmlList
{
    public $fields = [
        'checkbox' => [
            ...
            'options' => ['width' => '3%', 'id' => 'row_category_best_buy_id', 'data-row' => 'l.cdiscount_category_id'],
        ...
        'checkbox-del' => [
            'field' => 'cdiscount_category_id_del',
    ...
    public function getHtmlCdiscountCategoryIdDel($item, $fieldId)
    {
    ...
    $(\'.delete-button\').click(function (event) {
        ...
        $string = Tag::html('', 'input', ['value' => 'Delete', 'type' => 'button', 'class' => 'btn btn-default delete-button', 'data-value' => $item['cdiscount_category_id']], true);
...
//        }
    ...
    $(\'.refresh-button\').click(function (event) {
        var cdiscount_category_id =  $(this).attr(\'data-value\');
        $(\'#category_to_refresh\').val(cdiscount_category_id);
        $(\'#category-form\').submit();
    });';
        }
//        $string = Tag::html('', 'input', ['value' => 'Delete', 'type' => 'button', 'class' => 'btn btn-default delete-button', 'data-value' => $item['cdiscount_category_id']], true);

        $string = Tag::html('', 'input', ['value' => 'Refresh', 'type' => 'button', 'class' => 'btn btn-default refresh-button', 'data-value' => $item['cdiscount_category_id']]);

        $route ='cdiscount';
        $params = ['action' => 'syncCategory'];
        $options = ['query' => 'id='.$item['cdiscount_category_id']];

        $url = $this->controller->generateRouteUrl($route, $params, $options);


        $string.= Tag::html('manual sync', 'a', ['target' => 'blank', 'href' => $url]);
        return $string;
