...
{
    public static $BASE_URL = 'http://api.dbcapi.me/api';
    ...
    protected function _call($cmd, $payload = null)
    {
        ...
        $opts = [
            CURLOPT_URL => self::$BASE_URL . '/' . trim($cmd, '/'),
            CURLOPT_REFERER => '',
        ];
        if (null !== $payload) {
        ...
        if ($this->is_verbose) {
            fputs('php://stderr', time() . " SEND: {$cmd} " . serialize($payload) . "\n");
        }
        ...
        if ($this->is_verbose) {
            fputs('php://stderr', time() . " RECV: {$response}\n");
        }
        ...
        if (403 == $status_code) {
            ...
            if (400 == $status_code || 413 == $status_code) {
                throw new DeathByCaptcha_InvalidCaptchaException(
                ...
                if (503 == $status_code) {
                    ...
                    if (! ($response = call_user_func($this->_response_parser, $response))) {
...
class DeathByCaptcha_SocketClient extends DeathByCaptcha_Client
{
    ...
    protected function _call($cmd, $payload = null)
    {
        ...
        try {
            if (null === $response) {
                ...
                if (! ($response = $this->parse_json_response($response))) {
                    throw new DeathByCaptcha_ServerException(
                        'Invalid API response'
                    );
                }
    ...
    protected function _connect()
    {
        if (null === $this->_sock) {
            ...
            if (! ($sock = @fsockopen(self::HOST, $port, $errno, $error, self::DEFAULT_TIMEOUT))) {
                ...
                if (! @stream_set_timeout($sock, self::DEFAULT_TIMEOUT / 4)) {
                    fclose($sock);
                    throw new DeathByCaptcha_IOException(
    ...
    {
        if ($this->is_verbose) {
            fputs('php://stderr', time() . ' SEND: ' . strlen($buf) . ' ' . rtrim($buf) . "\n");
        }
            ...
            if (! $buf) {
                if (! ($s = fread($this->_sock, 4096))) {
                    ...
                    if (self::TERMINATOR == substr($s, strlen($s) - 2)) {
                        ...
                        if ($this->is_verbose) {
                            fputs('php://stderr', time() . ' RECV: ' . strlen($response) . " {$response}\n");
                        }
                        return $response;
                    }
                }
